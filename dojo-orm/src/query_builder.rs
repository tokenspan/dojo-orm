...
use crate::pagination::{Cursor, CursorValue, DefaultSortKeys, Pagination};
use crate::pool::*;
use crate::predicates::{Expr, ExprValueType, Predicate};
use crate::types::ToSql;
use crate::{Direction, OrderValue, Orders};
use typed_builder::TypedBuilder;

pub struct PagingExecution<'a, T>
where
    T: Model,
{
    pub pool: &'a Pool<PostgresConnectionManager<NoTls>>,
    pub params: &'a [&'a (dyn ToSql + Sync)],
    pub predicates: &'a [Predicate<'a>],
    pub orders: Vec<OrderValue>,
...
pub struct QueryBuilder<'a> {
    ...
    #[builder(default = &[])]
    pub predicates: &'a [Predicate<'a>],
    #[builder(default = Orders::default())]
    pub orders: Orders,
...
impl<'a> QueryBuilder<'a> {
    pub fn build_limit_sql(&self) -> String {
        ...
        let limit = self
            .limit
            .unwrap_or(self.first.unwrap_or(self.last.unwrap_or(20)));
    ...
    pub fn build_order_by_sql(&self) -> String {
        ...
        if self.ty != QueryType::Paging {
            let order_sql = self.orders.to_sql();
        ...
        if let Some(cursor) = self.before {
            let order_sql = cursor.to_order_by_stmt(Direction::Desc);
            stmt.push_str(" ORDER BY ");
            stmt.push_str(&order_sql);
        } else if let Some(cursor) = self.after {
            let order_sql = cursor.to_order_by_stmt(Direction::Asc);
            stmt.push_str(" ORDER BY ");
            stmt.push_str(&order_sql);
        } else {
            let order_sql = Cursor::order_by_stmt_by_keys(&self.default_keys, Direction::Desc);
            stmt.push_str(" ORDER BY ");
            stmt.push_str(&order_sql);
        }
...
mod tests {
    ...
    async fn test_build_first_after_with_additional_predicates_query() -> anyhow::Result<()> {
        let cursor = Some(cursor());
        let predicates = &[eq("name", &"test"), eq("age", &20)];
        let qb = QueryBuilder::builder()
            .table_name("users")
            .columns(&["id", "name", "age", "created_at"])
            .default_keys(vec!["created_at".to_string(), "id".to_string()])
    ...
    async fn test_build_where_by_id() -> anyhow::Result<()> {
        ...
        assert_eq!(
            stmt,
            "SELECT id, name, age, created_at FROM users WHERE id = $1 LIMIT 21"
        );
